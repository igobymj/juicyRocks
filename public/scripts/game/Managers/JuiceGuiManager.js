...
export default class JuiceGuiManager {
        ...
        this.schema = [
            ...
            },
            {
                ...
                children: [
                    {
                        label: "Track",
                        path: "container.music.track",
                        type: "select",
                        options: [
                            { value: "none", label: "None" },
                            { value: "heartbeat", label: "Heartbeat" },
                            { value: "media/music/techno-trance.mp3", label: "Techno-Trance" },
                            { value: "media/music/spy-theme.mp3", label: "Spy Theme" },
                            { value: "media/music/space-out.mp3", label: "Space Out" }
                        ]
                    },
                    {
                        label: "Volume",
    ...
    initialize() {
        ...
        // Apply initial track selection from settings
        const initialTrack = this.gameSession.juiceSettings.container.music.track;
        if (initialTrack && initialTrack !== 'none') {
            this._applyTrackSelection(initialTrack);
        }
    }
    ...
    setValue(path, value) {
        ...
        const isMusicJuice = path === 'container.music.track' && value !== 'none';
        ...
        // When juiceFx is toggled off, revert pewpewpew sounds and stop all music;
        // when toggled back on, re-apply the current track selection
        if (path === 'container.cheats.juiceFx') {
            ...
            const track = this.gameSession.juiceSettings.container.music.track;
            if (value) {
                this._applyTrackSelection(track);
            } else {
                this.gameSession.soundManager.stopHeartbeat();
                this.gameSession.soundManager.stopMusic();
            }
        }
        ...
        // Track selector (exclusive: heartbeat vs. music tracks vs. none)
        if (path === 'container.music.track') {
            this._applyTrackSelection(value);
        }
    ...
    // Apply a track selection value: stop everything, then start the chosen track
    _applyTrackSelection(track) {
        const sm = this.gameSession.soundManager;
        ...
        if (track === 'heartbeat') {
            sm.startHeartbeat();
        } else if (track && track !== 'none') {
            sm.selectTrack(track);
        }
    }
